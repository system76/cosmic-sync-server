...
use crate::{
    domain::{Account, Device, DeviceType, DomainService},
    ...
    pub async fn create_account_with_validation(
        ...
        if !self.is_valid_account_hash(&account_hash) {
            return Err(SyncError::Validation(
                "Invalid account hash format".to_string(),
            ));
        }
...
impl DeviceDomainService {
    ...
    pub async fn register_device_safely(
        ...
        if !self.is_valid_device_hash(&device_hash) {
            return Err(SyncError::Validation(
                "Invalid device hash format".to_string(),
            ));
        }
    ...
    /// 디바이스 중복 등록 방지
    pub async fn can_register_device(&self, account_hash: &str, device_hash: &str) -> Result<bool> {
...
mod tests {
    ...
    async fn test_account_domain_service() {
        ...
        // 유효한 계정 생성
        let result = service
            .create_account_with_validation(
    ...
    async fn test_device_domain_service() {
        ...
        // User-Agent 기반 추론
        assert_eq!(
            service.infer_device_type(
                "Unknown",
                Some("Mozilla/5.0 (iPhone; CPU iPhone OS 14_0 like Mac OS X)")
            ),
            DeviceType::Mobile
        );
    }
    ...
    fn test_file_sync_domain_service() {
        ...
        // 충돌 해결 전략 테스트
        let strategy = service.determine_conflict_resolution_strategy(1000, 900, 100, 100);
