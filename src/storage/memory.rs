...
use crate::storage::{Storage, StorageError, StorageMetrics, Result};
...
impl Storage for MemoryStorage {
    ...
    async fn create_account(&self, account: &Account) -> crate::storage::Result<()> {
        ...
        data.accounts.insert(account.account_hash.clone(), account.clone());
        Ok(())
    }
    ...
    /// Get account by hash
    async fn get_account_by_hash(&self, account_hash: &str) -> crate::storage::Result<Option<Account>> {
        let data = self.data.lock().await;
    ...
    async fn create_auth_token(&self, auth_token: &AuthToken) -> crate::storage::Result<()> {
        let mut data = self.data.lock().await;
        data.auth_tokens.insert(auth_token.access_token.clone(), auth_token.clone());
        Ok(())
    }
    ...
    /// Validate auth token
    async fn validate_auth_token(&self, token: &str, account_hash: &str) -> crate::storage::Result<bool> {
        let data = self.data.lock().await;
    ...
    /// Get device by hash
    async fn get_device(&self, account_hash: &str, device_hash: &str) -> crate::storage::Result<Option<Device>> {
        let data = self.data.lock().await;
    ...
    async fn list_devices(&self, account_hash: &str) -> crate::storage::Result<Vec<Device>> {
        ...
        let devices: Vec<Device> = data.devices.values()
    ...
    /// Delete device
    async fn delete_device(&self, account_hash: &str, device_hash: &str) -> crate::storage::Result<()> {
        let mut data = self.data.lock().await;
    ...
    /// Validate device
    async fn validate_device(&self, account_hash: &str, device_hash: &str) -> crate::storage::Result<bool> {
        let data = self.data.lock().await;
    ...
    async fn get_file_info_by_path(&self, account_hash: &str, file_path: &str, group_id: i32) -> crate::storage::Result<Option<FileInfo>> {
        let data = self.data.lock().await;
        
        for file in data.files.values() {
            if file.account_hash == account_hash && file.file_path == file_path && file.group_id == group_id {
    ...
    /// Get file by hash
    async fn get_file_by_hash(&self, account_hash: &str, file_hash: &str) -> crate::storage::Result<Option<FileInfo>> {
        let data = self.data.lock().await;
    ...
    /// List files
    async fn list_files(&self, account_hash: &str, group_id: i32, upload_time_from: Option<i64>) -> crate::storage::Result<Vec<FileInfo>> {
        let data = self.data.lock().await;
        
        let files: Vec<FileInfo> = data.files.values()
            .filter(|file| {
    ...
    async fn list_files_except_device(&self, account_hash: &str, group_id: i32, exclude_device_hash: &str, upload_time_from: Option<i64>) -> crate::storage::Result<Vec<FileInfo>> {
        let data = self.data.lock().await;
    ...
    async fn delete_file(&self, account_hash: &str, file_id: u64) -> crate::storage::Result<()> {
        ...
        if let Some(file) = data.files.get(&file_id) {
            ...
            if file.account_hash == account_hash {
                ...
                Err(StorageError::PermissionDenied(format!("File not owned by user: {}", account_hash)))
            }
        } else {
    ...
    /// Store file data
    async fn store_file_data(&self, file_id: u64, data_bytes: Vec<u8>) -> crate::storage::Result<()> {
        let mut data = self.data.lock().await;
    ...
    async fn store_encryption_key(&self, account_hash: &str, encryption_key: &str) -> crate::storage::Result<()> {
        let mut data = self.data.lock().await;
        data.encryption_keys.insert(account_hash.to_string(), encryption_key.to_string());
        Ok(())
    }
    
    /// Get encryption key
    async fn get_encryption_key(&self, account_hash: &str) -> crate::storage::Result<Option<String>> {
        let data = self.data.lock().await;
        Ok(data.encryption_keys.get(account_hash).cloned())
    }
    
    /// Register watcher group
    async fn register_watcher_group(&self, account_hash: &str, device_hash: &str, watcher_group: &WatcherGroup) -> crate::storage::Result<i32> {
        let mut data = self.data.lock().await;
    ...
    /// Get watcher groups
    async fn get_watcher_groups(&self, account_hash: &str) -> crate::storage::Result<Vec<WatcherGroup>> {
        let data = self.data.lock().await;
        
        let groups = data.watcher_groups.values()
    ...
    async fn get_user_watcher_group(&self, account_hash: &str, group_id: i32) -> crate::storage::Result<Option<WatcherGroup>> {
        let data = self.data.lock().await;
    ...
    async fn update_watcher_group(&self, account_hash: &str, watcher_group: &WatcherGroup) -> crate::storage::Result<()> {
        let mut data = self.data.lock().await;
    ...
    async fn get_watcher_group_by_account_and_id(&self, account_hash: &str, group_id: i32) -> crate::storage::Result<Option<WatcherGroupData>> {
        let data = self.data.lock().await;
    ...
    async fn register_watcher_preset_proto(&self, account_hash: &str, device_hash: &str, presets: Vec<String>) -> crate::storage::Result<()> {
        let mut data = self.data.lock().await;
        data.watcher_presets.insert(account_hash.to_string(), presets);
        Ok(())
    }
    
    /// Update watcher preset (proto호환 메소드)
    async fn update_watcher_preset_proto(&self, account_hash: &str, device_hash: &str, presets: Vec<String>) -> crate::storage::Result<()> {
        let mut data = self.data.lock().await;
        data.watcher_presets.insert(account_hash.to_string(), presets);
        Ok(())
    }
    ...
    /// 파일 경로와 이름으로 파일 찾기
    async fn find_file_by_path_and_name(&self, account_hash: &str, file_path: &str, filename: &str, revision: i64) -> crate::storage::Result<Option<FileInfo>> {
        let data = self.data.lock().await;
    ...
    /// 경로, 파일명, 그룹 ID로 파일 찾기
    async fn find_file_by_criteria(&self, account_hash: &str, group_id: i32, watcher_id: i32, file_path: &str, filename: &str) -> crate::storage::Result<Option<FileInfo>> {
        let data = self.data.lock().await;
        ...
        for file in data.files.values() {
            ...
            if file.account_hash == account_hash &&
               ...
               file.filename == filename {
                ...
                debug!("파일을 찾았습니다: file_id={}, path={}, group_id={}, watcher_id={}", 
                      file.file_id, file_path, group_id, watcher_id);
                return Ok(Some(file.clone()));
            }
        }
        
        debug!("파일을 찾을 수 없습니다: path={}, filename={}, group_id={}, watcher_id={}", 
    ...
    async fn create_watcher_with_conditions(&self, account_hash: &str, group_id: i32, watcher_data: &crate::sync::WatcherData, timestamp: i64) -> crate::storage::Result<i32> {
        ...
        let watcher = crate::models::watcher::Watcher {
            ...
            union_conditions: watcher_data.union_conditions.iter().map(|cd| crate::models::watcher::Condition {
                key: cd.key.clone(),
                value: cd.value.clone(), // ConditionData.value는 이미 Vec<String>
            }).collect(),
            subtracting_conditions: watcher_data.subtracting_conditions.iter().map(|cd| crate::models::watcher::Condition {
                key: cd.key.clone(),
                value: cd.value.clone(), // ConditionData.value는 이미 Vec<String>
            }).collect(),
        ...
        for condition in &watcher_data.union_conditions {
            ...
            data.watcher_conditions.insert(condition_id, watcher_condition);
        }
    ...
    async fn get_watcher_by_group_and_id(&self, account_hash: &str, group_id: i32, watcher_id: i32) -> crate::storage::Result<Option<crate::sync::WatcherData>> {
        let data = self.data.lock().await;
            ...
            if watcher.account_hash == account_hash &&
               ...
               watcher.watcher_id == watcher_id {
                ...
                let proto_watcher = crate::sync::WatcherData {
                    ...
                    union_conditions: watcher.union_conditions.iter().map(|c| crate::sync::ConditionData {
                        key: c.key.clone(),
                        value: c.value.clone(),
                    }).collect(),
                    subtracting_conditions: watcher.subtracting_conditions.iter().map(|c| crate::sync::ConditionData {
                        key: c.key.clone(),
                        value: c.value.clone(),
                    }).collect(),
    ...
    async fn get_server_group_id(&self, account_hash: &str, client_group_id: i32) -> crate::storage::Result<Option<i32>> {
        let data = self.data.lock().await;
    ...
    async fn get_server_ids(&self, account_hash: &str, client_group_id: i32, client_watcher_id: i32) -> crate::storage::Result<Option<(i32, i32)>> {
        let data = self.data.lock().await;
    ...
    /// 파일 정보 조회 (삭제된 파일 포함)
    async fn get_file_info_include_deleted(&self, file_id: u64) -> crate::storage::Result<Option<(FileInfo, bool)>> {
    ...
    async fn create_watcher_condition(&self, condition: &crate::models::watcher::WatcherCondition) -> crate::storage::Result<i64> {
        let mut data = self.data.lock().await;
    ...
    async fn get_watcher_conditions(&self, account_hash: &str, watcher_id: i32) -> crate::storage::Result<Vec<crate::models::watcher::WatcherCondition>> {
        let data = self.data.lock().await;
        
        let conditions: Vec<crate::models::watcher::WatcherCondition> = data.watcher_conditions
            .values()
            .filter(|condition| condition.watcher_id == watcher_id && condition.account_hash == account_hash)
            .cloned()
            .collect();
    ...
    async fn update_watcher_condition(&self, condition: &crate::models::watcher::WatcherCondition) -> crate::storage::Result<()> {
        let mut data = self.data.lock().await;
        
        let condition_id = condition.id.ok_or_else(|| {
            crate::storage::StorageError::ValidationError("Condition ID is required for update".to_string())
        })?;
        
        if data.watcher_conditions.contains_key(&condition_id) {
            data.watcher_conditions.insert(condition_id, condition.clone());
            Ok(())
        } else {
            Err(crate::storage::StorageError::NotFound(format!("Condition with ID {} not found", condition_id)))
        }
    }
    ...
    async fn save_watcher_conditions(&self, watcher_id: i32, conditions: &[crate::models::watcher::WatcherCondition]) -> crate::storage::Result<()> {
        let mut data = self.data.lock().await;
    ...
    /// 클라이언트 group_id로 서버 group_id 조회
    async fn get_client_watcher_id(&self, account_hash: &str, server_group_id: i32, server_watcher_id: i32) -> crate::storage::Result<Option<(i32, i32)>> {
    ...
    async fn get_watcher_condition(&self, condition_id: i64) -> crate::storage::Result<Option<crate::models::watcher::WatcherCondition>> {
        let data = self.data.lock().await;
        Ok(data.watcher_conditions.get(&condition_id).cloned())
    }
    
    async fn get_watcher_conditions_by_watcher(&self, account_hash: &str, watcher_id: i32) -> crate::storage::Result<Vec<crate::models::watcher::WatcherCondition>> {
        self.get_watcher_conditions(account_hash, watcher_id).await
    }
    
    // 중복 검사 메서드들
    async fn check_duplicate_watcher_group(&self, account_hash: &str, local_group_id: i32) -> crate::storage::Result<Option<i32>> {
        let data = self.data.lock().await;
    ...
    async fn check_duplicate_watcher(&self, account_hash: &str, local_watcher_id: i32) -> crate::storage::Result<Option<i32>> {
        let data = self.data.lock().await;
    ...
    async fn store_file_notice(&self, file_notice: &crate::models::file::FileNotice) -> crate::storage::Result<()> {
        // MemoryStorage에서는 FileNotice를 저장하지 않음 (임시 구현)
        Ok(())
    }
    
    async fn get_file_notices(&self, account_hash: &str, device_hash: &str) -> crate::storage::Result<Vec<crate::models::file::FileNotice>> {
        // MemoryStorage에서는 빈 리스트 반환 (임시 구현)
        Ok(Vec::new())
    }
    
    async fn delete_file_notice(&self, account_hash: &str, device_hash: &str, file_id: u64) -> crate::storage::Result<()> {
        // MemoryStorage에서는 아무 동작 없음 (임시 구현)
        Ok(())
    }

    // Version management methods implementation for MemoryStorage
    async fn get_file_history(&self, account_hash: &str, file_path: &str, group_id: i32) -> crate::storage::Result<Vec<crate::models::file::SyncFile>> {
        ...
        debug!("MemoryStorage: Getting file history for path: {} in group: {} (returning empty list)", file_path, group_id);
        Ok(Vec::new())
    }

    async fn get_file_versions_by_id(&self, account_hash: &str, file_id: u64) -> crate::storage::Result<Vec<crate::models::file::SyncFile>> {
        // For MemoryStorage, we return an empty list as this is primarily for testing
        debug!("MemoryStorage: Getting file versions for file_id: {} (returning empty list)", file_id);
        Ok(Vec::new())
    }

    async fn get_file_by_revision(&self, account_hash: &str, file_id: u64, revision: i64) -> crate::storage::Result<crate::models::file::SyncFile> {
        // For MemoryStorage, we return a NotFound error as this is primarily for testing
        debug!("MemoryStorage: Getting file by revision: file_id={}, revision={} (not implemented)", file_id, revision);
        Err(crate::storage::StorageError::NotFound(format!(
            "File version not found in MemoryStorage: file_id={}, revision={}", file_id, revision
        )))
    }
    ...
    async fn store_file(&self, file: &crate::models::file::SyncFile) -> crate::storage::Result<()> {
        ...
        debug!("MemoryStorage: Storing file version: file_id={}, revision={}", file.file_id, file.revision);
        Ok(())
    }

    async fn get_devices_for_account(&self, account_hash: &str) -> crate::storage::Result<Vec<Device>> {
        let data = self.data.lock().await;
        let devices: Vec<Device> = data.devices
            .values()
            .filter(|device| device.account_hash == account_hash)
            .cloned()
            .collect();
        
        debug!("MemoryStorage: Found {} devices for account: {}", devices.len(), account_hash);
        Ok(devices)
    }
