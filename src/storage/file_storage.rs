...
impl FileStorage for DatabaseFileStorage {
    async fn store_file_data(&self, file_id: u64, data: Vec<u8>) -> Result<String> {
        debug!("Storing file data in database: file_id={}, size={}", file_id, data.len());
    ...
    async fn file_data_exists(&self, file_id: u64) -> Result<bool> {
        debug!("Checking if file data exists in database: file_id={}", file_id);
...
impl S3FileStorage {
    ...
    async fn ensure_bucket_exists(&self) -> Result<()> {
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                Err(StorageError::S3Error(format!("Failed to check bucket existence: {}", e)))
            }
        }
    }
                ...
                if let aws_sdk_s3::error::SdkError::ServiceError(service_err) = &e {
                    match service_err.err() {
                        CreateBucketError::BucketAlreadyExists(_) => {
                            warn!("Bucket already exists (owned by another account): {}", self.config.bucket);
                            Err(StorageError::S3Error(format!("Bucket already exists: {}", self.config.bucket)))
                        }
                        CreateBucketError::BucketAlreadyOwnedByYou(_) => {
                            info!("Bucket already exists and is owned by you: {}", self.config.bucket);
                            Ok(())  // This is fine, race condition resolved
                        }
                        _ => {
                            error!("Failed to create bucket {}: {}", self.config.bucket, e);
                            Err(StorageError::S3Error(format!("Failed to create bucket: {}", e)))
                        }
                    }
                } else {
                    error!("Failed to create bucket {}: {}", self.config.bucket, e);
    ...
    fn is_minio(&self) -> bool {
        self.config.endpoint_url
            .as_ref()
            .map(|url| url.contains("minio") || url.contains("localhost") || url.contains("127.0.0.1"))
            .unwrap_or(false)
    }
...
impl FileStorage for S3FileStorage {
    async fn store_file_data(&self, file_id: u64, data: Vec<u8>) -> Result<String> {
        ...
        info!("Storing file data in S3: bucket={}, key={}, size={}", 
              self.config.bucket, s3_key, data.len());
            ...
            Err(e) => {
                ...
                if let StorageError::S3Error(ref msg) = e {
                    if msg.contains("NoSuchBucket") || msg.contains("BucketNotFound") {
                        warn!("Bucket not found during upload, attempting to create bucket and retry");
            ...
            Ok(response) => {
                ...
                match response.body.collect().await {
                    Ok(bytes) => {
                        ...
                        debug!("Successfully retrieved file from S3: {}, size={}", s3_key, data.len());
                        Ok(Some(data))
                    }
                    Err(e) => {
                        error!("Failed to read S3 response body: {}", e);
                        Err(StorageError::S3Error(format!("Failed to read response: {}", e)))
                    }
                }
            }
            Err(e) => {
                ...
                Err(StorageError::S3Error(format!("Failed to download file: {}", e)))
            }
        }
    }
    ...
    async fn file_data_exists(&self, file_id: u64) -> Result<bool> {
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                Err(StorageError::S3Error(format!("Failed to check file existence: {}", e)))
            }
        }
    }
    ...
    async fn get_file_size(&self, file_id: u64) -> Result<Option<u64>> {
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                Err(StorageError::S3Error(format!("Failed to get file size: {}", e)))
            }
        }
    }
    ...
    async fn health_check(&self) -> Result<()> {
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                return Err(StorageError::S3Error(format!("Bucket health check failed: {}", e)));
            }
        }
