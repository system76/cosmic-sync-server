...
impl MySqlFileExt for MySqlStorage {
    ...
    async fn store_file_info(&self, file_info: FileInfo) -> Result<u64> {
        ...
        if let Some((_existing_file_id, _current_revision)) = existing_by_file_id {
            ...
            // 트랜잭션 커밋
            tx.commit().await.map_err(|e| { error!("❌ 트랜잭션 커밋 실패(sqlx): {}", e); StorageError::Database(format!("트랜잭션 커밋 실패: {}", e)) })?;
    ...
    async fn delete_file(&self, account_hash: &str, file_id: u64) -> Result<()> {
        ...
        let (_, current_revision, file_path, filename, device_hash, group_id, watcher_id) = if let Some(rec) = file_exists {
            rec
        } else {
            return Err(StorageError::NotFound(format!("파일을 찾을 수 없음: {}", file_id)));
        };
        ...
        sqlx::query(r#"UPDATE files SET is_deleted = 1 
            ...
            .execute(&mut *tx)
            .await
            .map_err(|e| StorageError::Database(format!("이전 버전 파일 삭제 표시 실패(sqlx): {}", e)))?;
        ...
        // 트랜잭션 커밋
        tx.commit().await.map_err(|e| StorageError::Database(format!("트랜잭션 커밋 실패(sqlx): {}", e)))?;
    ...
    /// 파일 데이터 조회
    async fn get_file_data(&self, file_id: u64) -> Result<Option<Vec<u8>>> {
        let result: Option<Vec<u8>> = sqlx::query_scalar(r#"SELECT data FROM file_data WHERE file_id = ?"#)
            .bind(file_id as i64)
            .fetch_optional(self.get_sqlx_pool())
            .await
    ...
    /// 암호화 키 조회
    async fn get_encryption_key(&self, account_hash: &str) -> Result<Option<String>> {
        let result: Option<String> = sqlx::query_scalar(r#"SELECT encryption_key FROM encryption_keys WHERE account_hash = ?"#)
            .bind(account_hash)
            .fetch_optional(self.get_sqlx_pool())
            .await
            .map_err(|e| StorageError::Database(e.to_string()))?;
    ...
    async fn store_encryption_key(&self, account_hash: &str, encryption_key: &str) -> Result<()> {
        // 현재 시간 (초)
        let now = chrono::Utc::now().timestamp();
        let exists: Option<String> = sqlx::query_scalar(r#"SELECT account_hash FROM encryption_keys WHERE account_hash = ?"#)
            .bind(account_hash)
            .fetch_optional(self.get_sqlx_pool())
            .await
            .map_err(|e| StorageError::Database(e.to_string()))?;
    ...
    async fn check_file_exists(&self, file_id: u64) -> Result<(bool, bool)> {
        ...
        let is_deleted_opt: Option<i8> = sqlx::query_scalar(r#"SELECT is_deleted FROM files WHERE file_id = ?"#)
            .bind(file_id as i64)
            .fetch_optional(self.get_sqlx_pool())
            .await
            .map_err(|e| { error!("❌ 파일 존재 조회 SQL 오류(sqlx): {}", e); StorageError::Database(format!("파일 존재 조회 SQL 오류: {}", e)) })?;
