...
impl MySqlDeviceExt for MySqlStorage {
    ...
    async fn register_device(&self, device: &Device) -> Result<()> {
        ...
        .await
        .map_err(|e| { error!("Database error when checking for existing device: {}", e); StorageError::Database(e.to_string()) })?;

        if device_exists.is_some() {
            // 기존 장치가 존재하면 update 수행
            info!("Device already exists, updating: {}", device.device_hash);
            
            sqlx::query(r#"UPDATE devices SET 
                    device_name = ?, 
                    device_type = ?, 
                    os_type = ?, 
                    os_version = ?, 
                ...
                .execute(&mut *tx)
                .await
                .map_err(|e| { error!("Failed to update existing device: {}, error: {}", device.device_hash, e); StorageError::Database(e.to_string()) })?;
            ...
            sqlx::query(r#"INSERT INTO devices (
                ...
                .execute(&mut *tx)
                .await
                .map_err(|e| { error!("Failed to insert new device: {}, error: {}", device.device_hash, e); StorageError::Database(e.to_string()) })?;
            
            info!("Successfully registered new device: {}", device.device_hash);
        }

        // Commit transaction
        tx.commit().await.map_err(|e| { error!("Failed to commit device registration transaction: {}", e); StorageError::Database(format!("Transaction commit failed: {}", e)) })?;
    ...
    /// 장치 조회
    async fn get_device(&self, account_hash: &str, device_hash: &str) -> Result<Option<Device>> {
        let device_data: Option<(String, String, String, String, String, String, String, i64, i64, i64, bool)> = sqlx::query_as(
            r#"SELECT 
    ...
    /// 장치 목록 조회
    async fn list_devices(&self, account_hash: &str) -> Result<Vec<Device>> {
        let devices_data: Vec<(String, String, String, String, String, String, String, i64, i64, i64, bool)> = sqlx::query_as(
