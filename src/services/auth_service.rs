...
use crate::sync::{
    AuthNotificationResponse, AuthSuccessNotification, LoginResponse, OAuthExchangeResponse,
    VerifyLoginResponse,
};
...
impl AuthService {
    ...
    pub async fn validate_token(&self, token: &str, account_hash: &str) -> bool {
        ...
        match storage
            .as_ref()
            .validate_auth_token(token, account_hash)
            .await
        {
            Ok(valid) => valid,
            Err(e) => {
    ...
    pub async fn save_token(&self, auth_token: &AuthToken) -> Result<(), StorageError> {
        debug!(
            "AuthService: Saving token for account {}",
    ...
    pub async fn authenticate(
        ...
        email: &str,
        password: &str,
    ) -> Result<(Account, String), StorageError> {
