...
impl EncryptionService {
    ...
    pub async fn request_encryption_key(&self, account_hash: &str, device_hash: &str) -> Result<String, StorageError> {
        debug!("Processing encryption key request for account: {}, device: {}", account_hash, device_hash);
    ...
    pub async fn get_or_create_key(&self, account_hash: &str) -> Result<String, StorageError> {
        debug!("Getting or creating encryption key for account: {}", account_hash);
        
        match self.storage.get_encryption_key(account_hash).await {
            Ok(Some(key)) => {
                info!("Found existing encryption key for account: {}", account_hash);
                Ok(key)
            },
            Ok(None) => {
                // 키가 없으면 새로 생성
                info!("No encryption key found for account: {}, generating new key", account_hash);
                let new_key = self.generate_encryption_key();
                
                match self.storage.store_encryption_key(account_hash, &new_key).await {
                    Ok(_) => {
                        info!("Generated and stored new encryption key for account: {}", account_hash);
                        Ok(new_key)
                    },
                    Err(e) => {
    ...
    pub async fn update_encryption_key(&self, account_hash: &str, device_hash: &str, new_key: &str) -> Result<(), StorageError> {
        debug!("Processing encryption key update for account: {}, device: {}", account_hash, device_hash);
    ...
    pub async fn update_key(&self, account_hash: &str, new_key: &str) -> Result<(), StorageError> {
        ...
        if new_key.len() < ENCRYPTION_KEY_LENGTH {
            ...
            return Err(StorageError::ValidationError(
                format!("Encryption key must be at least {} characters", ENCRYPTION_KEY_LENGTH))
            );
        }
