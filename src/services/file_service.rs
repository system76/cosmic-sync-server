...
use crate::storage::{FileStorage, Result as StorageResult, Storage, StorageError};
use crate::sync;
...
impl FileService {
    ...
    pub fn with_storage_and_notifications(
        storage: Arc<dyn Storage>,
    ...
    fn prepare_metadata(&self, cfg: &AppConfig, mut fi: ModelFileInfo) -> ModelFileInfo {
        ...
        if let Some(kv) = key_vec_opt {
            if kv.len() == 32 {
                ...
                let ct_name =
                    crate::utils::crypto::aead_encrypt(key, fi.filename.as_bytes(), aad.as_bytes());
    ...
    pub async fn store_file(
        ...
        self.store_file_with_update_type(
            file_info,
            data,
            sync::file_update_notification::UpdateType::Uploaded,
        )
        .await
    }
    ...
    pub async fn convert_client_group_to_server(
        ...
        self.storage
            .get_server_group_id(account_hash, client_group_id)
            .await
    }
    ...
    pub async fn store_file_with_update_type(
        ...
        self.store_file_data_internal(file_info.file_id, data)
            .await?;
        ...
        self.files
            .lock()
            .unwrap()
            .insert(file_info.file_id, data.clone());
        debug!("✅ In-memory cache updated");

        // Send notification
        self.send_file_update_notification(file_info, update_type)
            .await;

        debug!(
            "🎉 FileService::store_file_with_update_type completed successfully for file_id={}",
            file_info.file_id
        );
        Ok(())
    }
    ...
    async fn store_file_data_internal(
        &self,
        file_id: u64,
        data: &Vec<u8>,
    ) -> Result<(), StorageError> {
        match self.file_storage.as_ref() {
            Some(file_storage) => match file_storage.store_file_data(file_id, data.clone()).await {
                Ok(_) => {
    ...
    async fn send_file_update_notification(
        ...
        self.send_file_update_notification_with_options(file_info, update_type, false)
            .await
    }
    ...
    async fn send_file_update_notification_with_options(
        ...
        if let Some(notification_manager) = &self.notification_manager {
            ...
            // 알림 전송 방식 선택
            debug!(
                "📤 Broadcasting file update notification (include_source: {})...",
                include_source_device
            );
            let result = if include_source_device {
                // 소스 장치도 포함해서 전송 (파일 복원 등)
                notification_manager
                    .broadcast_file_update_including_source(file_update_notification)
                    .await
            } else {
                // 소스 장치 제외하고 전송 (일반적인 파일 업로드)
                notification_manager
                    .broadcast_file_update(file_update_notification)
                    .await
            };
            ...
            match result {
                Ok(sent_count) => {
                    if sent_count > 0 {
                        info!(
                            "✅ File sync notification delivered to {} clients: {} ({}KB, rev {})",
                            sent_count,
                            file_info.filename,
                            file_info.size / 1024,
                            file_info.revision
                        ...
                        debug!(
                            "   → Notification details: file_id={}, account={}, path={}",
                            file_info.file_id, file_info.account_hash, file_info.file_path
                        );
                    } else {
                        warn!(
                            "⚠️ No active subscribers for file notification: {} (file_id={})",
    ...
    pub async fn copy_file_data(
        ...
        match source_data {
            Some(data) => {
                ...
                debug!(
                    "✅ File data copied successfully from {} to {}",
                    source_file_id, target_file_id
                );
                Ok(())
            }
    ...
    pub async fn restore_file_version(
        ...
        debug!(
            "🔄 Restoring file version: source={}, target={}",
            source_file_info.file_id, target_file_info.file_id
        );

        // Copy file data from source to target
        self.copy_file_data(source_file_info.file_id, target_file_info.file_id)
            .await?;

        // Update in-memory cache
        let file_data = self
            .get_file_data(target_file_info.file_id)
            .await?
            .unwrap_or_default();
        ...
        self.files
            .lock()
            .unwrap()
            .insert(target_file_info.file_id, file_data);
    ...
    pub async fn list_files_filtered_by_device(
        ...
        debug!(
            "Listing files with device filter: group_id={}, account_hash={}, device_hash={}",
        ...
        let files = self
            .storage
            .list_files_except_device(account_hash, group_id, device_hash, None)
            .await?;
        let result: Vec<FileInfoData> = files.into_iter().map(|f| f.into()).collect();

        debug!(
            "Filtered files for device {}: total={}",
            device_hash,
            result.len()
        );
    ...
    /// 파일 정보 조회 (삭제된 파일 포함)
    pub async fn get_file_info_include_deleted(
        &self,
        file_id: u64,
    ) -> Result<Option<(ModelFileInfo, bool)>, StorageError> {
    ...
    pub async fn delete_file(&self, file_id: u64) -> Result<(), StorageError> {
        ...
        match file_info_result {
            Some((file_info, is_deleted)) => {
                ...
                // 파일 삭제 처리
                debug!(
                    "파일 삭제 처리 시작: file_id={}, account_hash={}, file_path={}, filename={}",
                    file_id, file_info.account_hash, file_info.file_path, file_info.filename
                );

                match self
                    .storage
                    .delete_file(&file_info.account_hash, file_id)
                    .await
                {
                    Ok(_) => {
                        // 메모리 캐시에서 삭제
                        ...
                        if let Some(nm) = &self.notification_manager {
                            let notification = FileUpdateNotification {
                                ...
                                file_info: Some(file_info.to_sync_file()),
                                update_type:
                                    crate::sync::file_update_notification::UpdateType::Deleted
                                        as i32,
    ...
    pub async fn store_file_info(&self, file_info: &ModelFileInfo) -> Result<(), StorageError> {
        debug!(
            "Storing file info: file_id={}, filename={}",
            file_info.file_id, file_info.filename
        );
    ...
    pub async fn store_file_data(
        ...
        self.storage
            .store_file_data(file_id, file_data.clone())
            .await
    }
        ...
        if let Some(info) = file_info {
            ...
            if let Some(file_data) = data {
                ...
                self.files
                    .lock()
                    .unwrap()
                    .insert(file_id, file_data.clone());
    ...
    pub async fn find_file_by_local_path(
        ...
        debug!(
            "Service find_file_by_local_path - Original path: {}, Normalized path: {}",
            file_path, normalized_file_path
        );
    ...
    pub async fn find_file_by_criteria(
        ...
        debug!(
            "Searching file by criteria: account={}, group_id={}, watcher_id={}, path={}, name={}",
        ...
        let normalized_file_path = crate::utils::helpers::normalize_path_preserve_tilde(file_path);
        debug!(
            "Service find_file_by_criteria - Original path: {}, Normalized path: {}",
            file_path, normalized_file_path
        );
        ...
        if normalized_file_path.ends_with(&format!("/{}", file_name))
            || normalized_file_path.ends_with(file_name)
        {
            // 경로에 이미 파일명이 포함되어 있는 경우
            debug!(
                "경로가 이미 파일명을 포함하고 있습니다: {}",
                normalized_file_path
            );
            ...
            .await
        {
            ...
            Ok(None) => {
                debug!(
                    "파일을 찾을 수 없습니다: 경로={}, 이름={}, group_id={}, watcher_id={}",
    ...
    pub async fn check_file_exists_rpc(&self, file_id: u64) -> Result<bool, StorageError> {
        ...
        match self.storage.check_file_exists(file_id).await {
            ...
            Err(e) => {
                error!("Error checking file existence: {}", e);
                Err(StorageError::General(format!(
