...
impl ConfigLoader {
    ...
    pub async fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        info!(
            "Initializing config loader for environment: {:?}",
            environment
        );
    ...
    async fn parse_file_size(&self, key: &str, default: &str) -> usize {
        let size_str = self
            .get_config_value(key, Some(default))
            .await
        ...
        if self.environment.is_cloud() {
            if let Some(secret_name) = self.get_secret_name() {
                if let Some(value) = self
                    .get_from_secrets_manager_key_value(&secret_name, "server_encode_key")
                    .await
                {
                    return value;
                }
    ...
    async fn get_from_secrets_manager_key_value(
        &self,
        secret_name: &str,
        key: &str,
    ...
    pub async fn get_database_config(&self) -> super::settings::DatabaseConfig {
        ...
        let user = self
            .get_config_value("DB_USER", Some("user"))
            .await
            .unwrap_or_else(|| "user".to_string());
        let password = self
            .get_config_value("DB_PASS", Some("password"))
            .await
            .unwrap_or_else(|| "password".to_string());
        let name = self
            .get_config_value("DB_NAME", Some("cosmic_sync"))
            .await
            .unwrap_or_else(|| "cosmic_sync".to_string());
        let host = self
            .get_config_value("DB_HOST", Some("localhost"))
            .await
            .unwrap_or_else(|| "localhost".to_string());

        let port = self
            .get_config_value("DB_PORT", Some("3306"))
            .await
            .and_then(|p| p.parse::<u16>().ok())
            .unwrap_or(3306);
        let max_connections = self
            .get_config_value("DB_POOL", Some("5"))
            .await
            .and_then(|c| c.parse::<u32>().ok())
            .unwrap_or(5);
        let connection_timeout = self
            .get_config_value("DATABASE_CONNECTION_TIMEOUT", Some("30"))
            .await
            .and_then(|t| t.parse::<u64>().ok())
            .unwrap_or(30);
        let log_queries = self
            .get_config_value("DATABASE_LOG_QUERIES", Some("false"))
            .await
    ...
    pub async fn get_s3_config(&self) -> super::settings::S3Config {
        ...
        let region = self
            .get_config_value(
                "AWS_REGION",
                Some(crate::config::constants::DEFAULT_S3_REGION),
            )
            .await
        ...
        let bucket = self
            .get_config_value(
                "AWS_S3_BUCKET",
                Some(crate::config::constants::DEFAULT_S3_BUCKET),
            )
            .await
            .unwrap_or_else(|| crate::config::constants::DEFAULT_S3_BUCKET.to_string());
        let key_prefix = self
            .get_config_value("S3_KEY_PREFIX", Some("files/"))
            .await
        ...
        let force_path_style = self
            .get_config_value("S3_FORCE_PATH_STYLE", Some("false"))
            .await
            .map(|v| v == "1" || v.to_lowercase() == "true")
            .unwrap_or(false);

        let use_secret_manager = self
            .get_config_value("USE_AWS_SECRET_MANAGER", None)
            .await
            .map(|v| v == "1" || v.to_lowercase() == "true")
        ...
        let timeout_seconds = self
            .get_config_value("S3_TIMEOUT_SECONDS", Some("30"))
            .await
            .and_then(|v| v.parse().ok())
            .unwrap_or(30);
        let max_retries = self
            .get_config_value("S3_MAX_RETRIES", Some("3"))
            .await
            .and_then(|v| v.parse().ok())
            .unwrap_or(3);
    ...
    pub async fn get_server_config(&self) -> super::settings::ServerConfig {
        ...
        let host = self
            .get_config_value(
                "SERVER_HOST",
                Some(crate::config::constants::DEFAULT_GRPC_HOST),
            )
            .await
        ...
        let port = self
            .get_config_value(
                "GRPC_PORT",
                Some(&crate::config::constants::DEFAULT_GRPC_PORT.to_string()),
            )
            .await
            .and_then(|p| p.parse::<u16>().ok())
        ...
        let worker_threads = self
            .get_config_value(
                "WORKER_THREADS",
                Some(&crate::config::constants::DEFAULT_WORKER_THREADS.to_string()),
            )
            .await
            .and_then(|t| t.parse::<usize>().ok())
            .unwrap_or(crate::config::constants::DEFAULT_WORKER_THREADS);
        let auth_token_expiry_hours = self
            .get_config_value(
                "AUTH_TOKEN_EXPIRY_HOURS",
                Some(&crate::config::constants::DEFAULT_AUTH_TOKEN_EXPIRY_HOURS.to_string()),
            )
            .await
        ...
        let max_file_size = self
            .parse_file_size(
                "MAX_FILE_SIZE",
                &crate::config::constants::DEFAULT_MAX_FILE_SIZE_BYTES.to_string(),
            )
            .await;
        ...
        let max_concurrent_requests = self
            .get_config_value(
                ...
                Some(&crate::config::constants::DEFAULT_MAX_CONCURRENT_REQUESTS.to_string()),
            )
            .await
        ...
        let heartbeat_interval_secs = self
            .get_config_value(
                ...
                Some(&crate::config::constants::DEFAULT_HEARTBEAT_INTERVAL_SECS.to_string()),
            )
            .await
    ...
    pub async fn get_storage_config(&self) -> super::settings::StorageConfig {
        ...
        let storage_type = self
            .get_config_value("STORAGE_TYPE", Some("database"))
            .await
        ...
        let key_prefix = self
            .get_config_value(
                "S3_KEY_PREFIX",
                Some(crate::config::constants::DEFAULT_S3_KEY_PREFIX),
            )
            .await
        ...
        let force_path_style = self
            .get_config_value(
                "S3_FORCE_PATH_STYLE",
                Some(if crate::config::constants::DEFAULT_S3_FORCE_PATH_STYLE {
                    "true"
                } else {
                    "false"
                }),
        ...
        let use_secret_manager = self
            .get_config_value(
                ...
                Some(if crate::config::constants::DEFAULT_S3_USE_SECRET_MANAGER {
                    "true"
                } else {
                    "false"
                }),
        ...
        let timeout_seconds = self
            .get_config_value(
                "S3_TIMEOUT_SECONDS",
                Some(&crate::config::constants::DEFAULT_S3_TIMEOUT_SECONDS.to_string()),
            )
            .await
            .and_then(|v| v.parse().ok())
        ...
        let max_retries = self
            .get_config_value(
                "S3_MAX_RETRIES",
                Some(&crate::config::constants::DEFAULT_S3_MAX_RETRIES.to_string()),
            )
            .await
            .and_then(|v| v.parse().ok())
            .unwrap_or(crate::config::constants::DEFAULT_S3_MAX_RETRIES);

        let file_ttl_secs = self
            .get_config_value(
                "FILE_TTL_SECS",
                Some(&crate::config::constants::DEFAULT_FILE_TTL_SECS.to_string()),
            )
            .await
            .and_then(|v| v.parse::<i64>().ok())
            .unwrap_or(crate::config::constants::DEFAULT_FILE_TTL_SECS);
        let max_file_revisions = self
            .get_config_value(
                "MAX_FILE_REVISIONS",
                Some(&crate::config::constants::DEFAULT_MAX_FILE_REVISIONS.to_string()),
            )
            .await
    ...
    pub async fn get_logging_config(&self) -> super::settings::LoggingConfig {
        ...
        let level = self
            .get_config_value("LOG_LEVEL", Some("info"))
            .await
            .unwrap_or_else(|| "info".to_string());
        let file_logging = self
            .get_config_value("LOG_TO_FILE", Some("true"))
            .await
            .map(|v| v == "1" || v.to_lowercase() == "true")
            .unwrap_or(true);
        let log_file = self
            .get_config_value("LOG_FILE", Some("logs/cosmic-sync-server.log"))
            .await
            .unwrap_or_else(|| "logs/cosmic-sync-server.log".to_string());
        let max_file_size = self
            .get_config_value("LOG_MAX_FILE_SIZE", Some("10485760"))
            .await // 10MB
            .and_then(|s| s.parse::<usize>().ok())
            .unwrap_or(10 * 1024 * 1024);
        let max_backups = self
            .get_config_value("LOG_MAX_BACKUPS", Some("5"))
            .await
            .and_then(|b| b.parse::<usize>().ok())
            .unwrap_or(5);
        let format = self
            .get_config_value("LOG_FORMAT", Some("text"))
            .await
    ...
    pub async fn get_feature_flags(&self) -> super::settings::FeatureFlags {
        ...
        let test_mode = self
            .get_config_value("COSMIC_SYNC_TEST_MODE", Some("false"))
            .await
            .map(|v| v == "1" || v.to_lowercase() == "true")
            .unwrap_or(false);
        let debug_mode = self
            .get_config_value("COSMIC_SYNC_DEBUG_MODE", Some("false"))
            .await
            .map(|v| v == "1" || v.to_lowercase() == "true")
            .unwrap_or(false);
        let metrics_enabled = self
            .get_config_value("ENABLE_METRICS", Some("false"))
            .await
            .map(|v| v == "1" || v.to_lowercase() == "true")
            .unwrap_or(false);
        let storage_encryption = self
            .get_config_value("STORAGE_ENCRYPTION", Some("true"))
            .await
            .map(|v| v == "1" || v.to_lowercase() == "true")
            .unwrap_or(true);
        let request_validation = self
            .get_config_value("REQUEST_VALIDATION", Some("true"))
            .await
            .map(|v| v == "1" || v.to_lowercase() == "true")
            .unwrap_or(true);
        let transport_encrypt_metadata = self
            .get_config_value("COSMIC_TRANSPORT_ENCRYPT_METADATA", Some("true"))
            .await
            .map(|v| v == "1" || v.to_lowercase() == "true")
            .unwrap_or(true);
        let dev_mode = self
            .get_config_value("COSMIC_SYNC_DEV_MODE", Some("false"))
            .await
            .map(|v| v == "1" || v.to_lowercase() == "true")
            .unwrap_or(false);
