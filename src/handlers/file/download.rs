...
fn parse_account_key(s: &str) -> Option<[u8;32]> {
    if let Ok(bytes) = hex::decode(s) {
        if bytes.len() == 32 { return Some(bytes.try_into().ok()?); }
    }
...
pub async fn handle_download_file(handler: &FileHandler, req: DownloadFileRequest) -> Result<Response<DownloadFileResponse>, Status> {
    ...
    let account_key: Option<[u8;32]> = if handler.app_state.config.features.transport_encrypt_metadata {
        match handler.app_state.storage.get_encryption_key(&file_info.account_hash).await {
    ...
    match handler.app_state.file.get_file_data(file_id).await {
        Ok(Some(data)) => {
            ...
            info!("Sending file download response: file_id={}, filename={}, file_path= {}", file_id, enc_name, enc_path);
