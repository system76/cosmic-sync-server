...
pub async fn handle_list_files(
    ...
    debug!(
        "File list request: account={}, device={}, group_id={}, time_filter={:?}",
    ...
    if handler
        ...
        .storage
        .as_any()
        .downcast_ref::<crate::storage::memory::MemoryStorage>()
        .is_some()
    {
    ...
    let server_group_id = match handler
        .app_state
        .file
        .convert_client_group_to_server(&req.account_hash, req.group_id)
        .await
    {
        Ok(Some(id)) => id,
        Ok(None) => {
            debug!(
                "No matching group found for client group_id={}",
                req.group_id
            );
            return Ok(Response::new(ListFilesResponse {
                success: true,
    ...
    debug!(
        "Converted client group_id={} to server group_id={}",
        req.group_id, server_group_id
    );
    ...
    if let Some(ref filter_time) = time_filter {
        info!(
            "🔍 Recovery sync requested: filtering files updated since {}",
            filter_time
        );
    }
    ...
    let account_key: Option<[u8; 32]> =
        if handler.app_state.config.features.transport_encrypt_metadata {
            match handler
                .app_state
                .storage
                .get_encryption_key(&req.account_hash)
                .await
            {
    ...
    match handler
        .app_state
        .file
        .list_files_filtered_by_device(server_group_id, &req.account_hash, &req.device_hash)
        .await
    {
        Ok(files) => {
            ...
            for file in files.iter() {
                ...
                if let Some(ref filter_time) = time_filter {
                    let file_updated_time = timestamp_to_datetime(&prost_types::Timestamp {
                    ...
                    debug!(
                        "🔄 Including file {} (updated: {}) in recovery sync",
                        file.filename, file_updated_time
                    );
                }

                let client_watcher_id = match handler
                    .app_state
                    .storage
                    .get_client_watcher_id(&req.account_hash, server_group_id, file.watcher_id)
                    .await
                {
                    Ok(Some((_, watcher_id))) => watcher_id,
                    Ok(None) => {
                        warn!(
                            "No client watcher_id found for server watcher_id={}",
                            file.watcher_id
                        );
                        file.watcher_id
                    }
