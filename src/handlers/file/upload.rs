...
pub async fn handle_upload_file(
    ...
    if req.file_data.len() != req.file_size as usize {
        error!(
            "File data size mismatch: declared={}, actual= {}",
            req.file_size,
            req.file_data.len()
        );
        return Ok(Response::new(response::file_upload_error(
    ...
    if let Err(msg) = handler
        .validate_file_path_with_watcher(&req, &normalized_file_path)
        .await
    {
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            return Ok(Response::new(response::file_upload_error(format!(
                "Failed to ensure server IDs: {}",
                e
            ))));
        }
    };
    ...
    let file_info = handler.app_state.file.build_file_info_for_upload(
        &req_server,
        file_id,
    ...
    match handler
        .app_state
        .file
        .store_file(&file_info, &req.file_data)
        .await
    {
        Ok(_) => {
            ...
            let routing_key = format!(
                "file.uploaded.{}.{}.{}",
                server_account_hash, server_group_id, server_watcher_id
            );
            ...
            let routing_key = format!("version.created.{}.{}", server_account_hash, file_id);
            let payload = serde_json::json!({
            ...
            Ok(Response::new(response::file_upload_success(
                file_id,
                req.revision + 1,
            )))
        }
        Err(e) => {
            ...
            Ok(Response::new(response::file_upload_error(&format!(
                "File storage failed: {}",
                e
            ))))
        }
