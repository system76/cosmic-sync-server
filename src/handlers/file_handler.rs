use crate::sync::{
    AuthUpdateNotification, CheckFileExistsRequest, CheckFileExistsResponse, DeleteFileRequest,
    DeleteFileResponse, DeviceUpdateNotification, DownloadFileRequest, DownloadFileResponse,
    EncryptionKeyUpdateNotification, FileUpdateNotification, FindFileRequest, FindFileResponse,
    ListFilesRequest, ListFilesResponse, UploadFileRequest, UploadFileResponse,
...
impl FileHandler {
    ...
    pub(crate) fn normalize_file_path(&self, file_path: &str) -> Result<String, String> {
        ...
        if normalized_file_path.len() > 1024 {
            error!(
                "Normalized path too long: {} chars (max 1024)",
                normalized_file_path.len()
            );
    ...
    pub(crate) async fn validate_file_path_with_watcher(
        &self,
        ...
        let watcher = match self
            .get_watcher_info_once(&req.account_hash, req.group_id, req.watcher_id)
            .await
        {
            Ok(watcher) => watcher,
            Err(e) => {
        ...
        let normalized_watcher_folder =
            crate::utils::helpers::normalize_path_preserve_tilde(&watcher.folder);
        ...
        if !file_is_in_watcher_folder {
            ...
            return Err(format!(
                "File path '{}' is not within watcher folder '{}'",
                normalized_file_path, normalized_watcher_folder
            ));
        }
    ...
    /// get watcher info once (no retry)
    async fn get_watcher_info_once(
        &self,
        account_hash: &str,
        group_id: i32,
        watcher_id: i32,
    ) -> Result<crate::sync::WatcherData, String> {
        debug!(
            "Getting watcher info once: account={}, group_id={}, watcher_id={}",
        ...
        match self
            .app_state
            .storage
            .get_watcher_by_group_and_id(account_hash, group_id, watcher_id)
            .await
        {
            Ok(Some(watcher)) => {
                debug!(
                    "Watcher found: folder={}, recursive={}",
                    watcher.folder, watcher.recursive_path
                );
                Ok(watcher)
            }
            Ok(None) => {
    ...
    pub async fn delete_file_internal(
        &self,
    ...
    pub(crate) async fn validate_file_for_deletion(
        &self,
        file_id: u64,
    ) -> Result<u64, Response<DeleteFileResponse>> {
        ...
        match self.app_state.file.check_file_exists(file_id).await {
            Ok((exists, is_deleted)) => {
                info!(
                    "File status check: file_id={}, exists={}, is_deleted={}",
                    file_id, exists, is_deleted
                );
                if !exists {
                ...
                if is_deleted {
                    ...
                    return Err(Response::new(response::file_delete_success(
            ...
            Err(e) => {
                ...
                Err(Response::new(response::file_delete_error(format!(
    ...
    pub(crate) async fn find_file_by_path(
        ...
        debug!(
            "Searching file by name: filename={}, path={}",
            req.filename, req.file_path
        );

        let normalized_file_path =
            crate::utils::helpers::normalize_path_preserve_tilde(&req.file_path);
        debug!(
            "Path normalized: original={}, normalized={}",
            ...
            .await
        {
            Ok(Some(info)) => {
                debug!(
                    "File found: file_id={}, filename={}",
                    info.file_id, info.filename
                );
                Ok(info.file_id)
            }
            Ok(None) => {
                warn!(
                    "File not found: filename={}, path={}",
                    req.filename, normalized_file_path
                );
                Err(Response::new(response::file_delete_error(format!(
                    "File not found: {}",
                    req.filename
