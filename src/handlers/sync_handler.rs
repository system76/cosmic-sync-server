...
use crate::sync::{
    ...
    VersionUpdateNotification, GetFileHistoryRequest, GetFileHistoryResponse,
...
impl SyncHandler {
    ...
    pub async fn request_encryption_key(&self, request: Request<RequestEncryptionKeyRequest>) -> Result<Response<RequestEncryptionKeyResponse>, Status> {
        let req = request.into_inner();
        debug!("Encryption key request: account={}, device={}", req.account_hash, req.device_hash);
        ...
        match self.app_state.encryption.get_or_create_key(&auth_result.account_hash).await {
            Ok(key) => {
                info!("Encryption key provided for account: {}", auth_result.account_hash);
    ...
    pub async fn health_check(&self, _request: Request<HealthCheckRequest>) -> Result<Response<HealthCheckResponse>, Status> {
    ...
    pub async fn get_file_history(&self, request: Request<GetFileHistoryRequest>) -> Result<Response<GetFileHistoryResponse>, Status> {
        let req = request.into_inner();
        debug!("Get file history request: account={}, path={}", req.account_hash, req.file_path);
        ...
        match self.app_state.version_service.get_file_history(req).await {
            Ok(response) => {
                info!("File history retrieved for account: {}", auth_result.account_hash);
    ...
    pub async fn restore_file_version(&self, request: Request<RestoreFileVersionRequest>) -> Result<Response<RestoreFileVersionResponse>, Status> {
        let req = request.into_inner();
        debug!("Restore file version request: account={}, file_id={}, revision={}", 
        ...
        match self.app_state.version_service.restore_file_version(req).await {
            Ok(response) => {
                info!("File version restored for account: {}", auth_result.account_hash);
                Ok(Response::new(response))
            },
            Err(e) => {
    ...
    pub async fn broadcast_file_restore(&self, request: Request<BroadcastFileRestoreRequest>) -> Result<Response<BroadcastFileRestoreResponse>, Status> {
        let req = request.into_inner();
        debug!("Broadcast file restore request: account={}, file_id={}", 
