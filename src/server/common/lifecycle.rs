use super::{Component, ComponentStatus, ComponentHealth};
use super::{StatusManager, MetricsCollector, HealthMonitor};
use std::sync::Arc;
...
impl<T: Component> ServiceManager<T> {
    ...
    pub async fn start(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.status_manager.set_status(ComponentStatus::Starting).await;
        ...
        // Start the actual component
        {
            ...
            if let Err(e) = component.start().await {
                ...
                error!("{}", error_msg);
                self.status_manager.set_status(ComponentStatus::Error(error_msg)).await;
                return Err(e.into());
            }
        }
        
        // Update status to running
        self.status_manager.set_status(ComponentStatus::Running).await;
    ...
    pub async fn stop(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.status_manager.set_status(ComponentStatus::Stopping).await;
        ...
        self.status_manager.set_status(ComponentStatus::Stopped).await;
    ...
    pub async fn update_config(&mut self, config: T::Config) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn start_monitoring_tasks(&mut self) {
        ...
        self.task_manager.add_task("metrics_collection", metrics_task).await;
        ...
        self.task_manager.add_task("health_monitoring", health_task).await;
    }
}
