...
use crate::{
    sync::{
        ...
        sync_client_service_server::SyncClientServiceServer,
    },
    server::{
        service::{SyncServiceImpl, SyncClientServiceImpl},
        app_state::AppState,
    },
...
async fn start_grpc_server(
    ...
    let server = {
        let reflection_service = tonic_reflection::server::Builder::configure()
            .register_encoded_file_descriptor_set(tonic::include_file_descriptor_set!("sync_descriptor"))
            .build()
            .map_err(|e| SyncError::Internal(format!("Failed to create reflection service: {}", e)))?;
...
async fn start_http_server(
    ...
    HttpServer::new(move || {
        ...
        App::new()
            ...
            .route("/health/live", web::get().to(handlers::health::liveness_check))
            
            // Metrics endpoints
            .route("/metrics", web::get().to(handlers::metrics::prometheus_metrics))
            .route("/metrics/detailed", web::get().to(handlers::metrics::detailed_metrics))
            
            // OAuth endpoints
            .route("/oauth/login", web::get().to(handlers::oauth::handle_oauth_login))
            .route("/oauth/callback", web::get().to(handlers::oauth::handle_oauth_callback))
...
fn parse_mysql_url(url: &str) -> Result<crate::config::settings::DatabaseConfig> {
    let url = url::Url::parse(url)
        .map_err(|e| SyncError::Config(format!("Invalid MySQL URL: {}", e)))?;
    ...
    if database.is_empty() {
        return Err(SyncError::Config("Missing database name in MySQL URL".to_string()));
    }
...
async fn graceful_shutdown(app_state: Arc<AppState>) -> Result<()> {
    ...
    while app_state.connection_tracker.get_total_active_connections().await > 0 && wait_count < 100 {
        tokio::time::sleep(Duration::from_millis(100)).await;
        wait_count += 1;
    }
    
    if app_state.connection_tracker.get_total_active_connections().await > 0 {
        warn!("⚠️ {} connections still active during shutdown", app_state.connection_tracker.get_total_active_connections().await);
    }
